...
#pragma warning disable 649
        ...
        [SerializeField] private Transform camera_without_gun;
...
#pragma warning restore 649
        ...
        private void Start()
        {
            ...
            camera_without_gun = AssignCharactersCamera1();
        ...
        {
            var t = transform;
            camera_without_gun.SetPositionAndRotation(t.position, t.rotation);
			return camera_without_gun;
        }
        ...
        private void Update()
        {
            ...
            camera_without_gun.position = transform.position + transform.TransformVector(armPosition);
            Jump();
            PlayFootstepSounds();
        }
        ...
        private void RotateCameraAndCharacter1()
        {
            var rotationX = _rotationX.Update(RotationXRaw, rotationSmoothness);
            var rotationY = _rotationY.Update(RotationYRaw, rotationSmoothness);
            var clampedY = RestrictVerticalRotation1(rotationY);
            _rotationY.Current = clampedY;
			var worldUp = camera_without_gun.InverseTransformDirection(Vector3.up);
			var rotation = camera_without_gun.rotation *
                           Quaternion.AngleAxis(rotationX, worldUp) *
                           Quaternion.AngleAxis(clampedY, Vector3.left);
            transform.eulerAngles = new Vector3(0f, rotation.eulerAngles.y, 0f);
            camera_without_gun.rotation = rotation;
        }
        ...
        private float RestrictVerticalRotation1(float mouseY)
        {
			var currentAngle = NormalizeAngle(camera_without_gun.eulerAngles.x);
            var minY = minVerticalAngle + currentAngle;
            var maxY = maxVerticalAngle + currentAngle;
            return Mathf.Clamp(mouseY, minY + 0.01f, maxY - 0.01f);
        }
